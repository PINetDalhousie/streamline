# sample command: sudo python3 evaluation/accuracy-analysis/military-coordination/scripts/testbedScripts/modifiedLatencyPlotScript.py --log-dir evaluation/accuracy-analysis/military-coordination/logs/testbed-logs/round1
#!/usr/bin/python3
...
def readConsumerData(prodDetails, consDetails, nProducer, nConsumer, logDir):
    ...
    for cons in consDetails:
        # print('Start reading data from: '+logDir+'cons/'+'cons-node'+str(cons['consNodeID'])\
...
def parse_time(time_str):
    try:
        # Try to parse the time string with the fractional part
        return datetime.strptime(time_str, "%H:%M:%S.%f")
    except ValueError:
        # If that fails, try to parse it without the fractional part
    ...
    with open(logDir+"/latency-log.txt", "r") as f:
        for lineNum, line in enumerate(f,1):         #to get the line number
            ...
            if "Latency of this message: " in line:
                ...
                # taking microseconds into account
                
                # Parse the time string
                firstSplit = line.split("Latency of this message: ")
                # print("line :"+str(lineNum)+" "+firstSplit[1])
                # Parse the time string
                firstSplit = line.split("Latency of this message: ")
                time_str = firstSplit[1].strip()  # Remove leading/trailing whitespace
                time_str = time_str.split(" ")[0]  # Keep only the first part of the string before any space

                time_obj = parse_time(time_str)

                # Convert the time to seconds
                seconds = time_obj.hour * 3600 + time_obj.minute * 60 + time_obj.second + time_obj.microsecond / 1e6
                latencyYAxis.append(float(seconds))
...
def plotConsTopicLactencyScatter(nConsumer):
    consumersLatency = []
    timesSent = []
    prodIDs = []
    topicIDs = []
...
def plotLatencyScatterSorted(nConsumer):
    consumersLatency = []
    timesSent = []
    for consId in range(nConsumer):
        consumersLatency.append({})
...
# Create a dictionary of component details
def initComponentDetails(nComponent, nComponentInstance, compName): 
    i=1
    componentDetails = []
    while i<=nComponent:
        j = 1
        while j<=nComponentInstance:
            compDict = {compName+'NodeID':i, compName+'InstID':j}
            componentDetails.append(compDict)
            j+=1
        i+=1
    return componentDetails

def latencyAverage(lst): 
    latencyAvg =  sum(lst) / len(lst)
    print('Total number of messages: '+str(len(lst)))
    return latencyAvg

# # user-input
parser = argparse.ArgumentParser(description='Script for measuring latency for each message.')
parser.add_argument('--number-of-producers', dest='nProducer', type=int, default=4, help='Number of producers')
parser.add_argument('--instance-in-each-producer', dest='nProducerInstance', type=int, default=1, help='Number of instances in each producer node')
parser.add_argument('--number-of-consumers', dest='nConsumer', type=int, default=4, help='Number of consumers')
parser.add_argument('--instance-in-each-consumer', dest='nConsumerInstance', type=int, default=1, help='Number of instances in each consumer node')
...
prodDetails = initComponentDetails(args.nProducer, args.nProducerInstance, 'prod')
consDetails = initComponentDetails(args.nConsumer, args.nConsumerInstance, 'cons')
...
# print('Latency Y Axis: ', latencyYAxis)
latencyAvg = latencyAverage(latencyYAxis)
print("Average latency: " + str(latencyAvg)+" seconds")

plotLatencyScatterSorted(nConsumer)
clearExistingPlot()
...
plotConsTopicLactencyScatter(nConsumer)
clearExistingPlot()
